$(document).ready(function () {
    ...
    $('#txt_buscarPago').keyup(function (e) {//permite hacer busqueda de cursos
        if ($('#txt_buscarPago').val()) {
            let buscar = $('#txt_buscarPago').val().toUpperCase();
            let plantilla = '';
            $.ajax({
    ...
    function MostrarTabla(plantilla, cur) {//////Mostrar Tabla///////////
        plantilla +=
            `<tr UserDocu="${cur.pacodcur}" class="table-light">
                ...
                <td>${cur.cadescur}</td>
                <td>
                    <button class="generarPago btn btn-primary">
                    <i class="fas fa-clipboard-list"></i></button>
                </>
            </tr>`
        return plantilla;
    }
    $(document).on('click', '.generarPago', function () {//elimina miembros

        let elemento = $(this)[0].parentElement.parentElement;
        let pacodcur = $(elemento).attr('UserDocu');
        abrirNuevoTab('/MRFSistem/ReportesPDF/PDF_ControlPago.php?pacodcur=' + pacodcur);
